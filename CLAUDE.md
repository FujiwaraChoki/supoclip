...
- **Metadata (title/description/tags) is generated once per video, not per clip.** `metadata_generation.py::generate_metadata_for_video` sends every clip's transcript excerpt in a single prompt and maps the response back by `clip_index`; a separate, lighter `generate_metadata_for_single_clip` exists only for the per-clip "Regenerate" button, so that action doesn't re-run the whole video's batch. Results cache on `generated_clips.metadata_*` columns (never regenerated on render/preview/re-open) alongside `metadata_provider`/`metadata_generated_at`/`metadata_generation_ms` for the UI's provider badge and timing display.
