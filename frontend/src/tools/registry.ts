 ...
 *      straight into a container (see `placeholder/index.tsx`).
 *   2. If it's route-based, add the route(s) under `frontend/src/app/`
 *      (its own route group, e.g. `(ranking)/`, keeps it visually
 ...
 * to change.
 */
export const TOOLS: Tool[] = [clippingTool, placeholderTool];
